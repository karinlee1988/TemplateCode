...
class VlookupGui(object):
    ...
    def window(self):
        ...
        tk.Label(self.master,text="（给大笨猪小娟用的=。=）",font=("黑体",16)).pack()
    ...
    def frame(self):
        ...
        tk.Label(frame1, text="模板工作薄->", font=("黑体", 16)).grid(row=2, column=0)
        ...
        tk.Button(frame1, text="选择模板工作薄", command=self.select_temp_path, font=("黑体", 16)).grid(row=2, column=2)
        tk.Button(frame1, text="选择数据工作薄", command=self.select_source_path, font=("黑体", 16)).grid(row=3, column=2)

        tk.Label(frame2, text="模板工作薄工作表序号", font=("黑体", 16)).grid(row=1, column=0)
        tk.Entry(frame2, textvariable=self.v1).grid(row=1, column=1)
        tk.Label(frame2, text="数据工作薄工作表序号:", font=("黑体", 16)).grid(row=2, column=0)
        tk.Entry(frame2, textvariable=self.v2).grid(row=2, column=1)
        tk.Label(frame2, text="模板工作薄要查找的列：", font=("黑体", 16)).grid(row=3, column=0)
        tk.Entry(frame2, textvariable=self.v3).grid(row=3, column=1)
        tk.Label(frame2, text="模板工作薄要填写的列:", font=("黑体", 16)).grid(row=4, column=0)
        tk.Entry(frame2, textvariable=self.v4).grid(row=4 ,column=1)
        tk.Label(frame2, text="数据工作薄对应模板工作薄的列:", font=("黑体", 16)).grid(row=5, column=0)
        tk.Entry(frame2, textvariable=self.v5).grid(row=5, column=1)
        tk.Label(frame2, text="数据工作薄数据列:", font=("黑体", 16)).grid(row=6, column=0)
        tk.Entry(frame2, textvariable=self.v6).grid(row=6 ,column=1)


        # 按这个按钮执行主程序
        tk.Button(frame2, text="开始处理", command=self.main, font=("黑体", 16)).grid(row=7, column=1,pady=33)
        tk.Entry(frame2, textvariable=self.flag,state="readonly").grid(row=8, column=1)
    ...
    def main(self):
        ...
        template_key = self.v3.get()
        template_value = self.v4.get()

        ws_source_index = int(self.v2.get()) - 1
        ...
        wb_template.save(self.temp_path.get().replace('.xlsx','')+'(已处理).xlsx')
        # 标志设置为处理完成
        self.flag.set("处理完成！")
